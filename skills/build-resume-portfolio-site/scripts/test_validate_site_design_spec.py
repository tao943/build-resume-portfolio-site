...
def valid_spec_v4() -> dict[str, object]:
    payload = valid_spec()
    payload["schema_version"] = 4
    payload["decisions"]["structure"] = {
...
class SiteDesignSpecValidatorTests(unittest.TestCase):
    ...
    def test_accepts_v4_agent_delegated_structure(self) -> None:
        self.assertEqual(self.run_validator(valid_spec_v4()).returncode, 0)

    def test_rejects_v4_structure_with_selected_seed(self) -> None:
        payload = valid_spec_v4()
        payload["decisions"]["structure"]["selected_seed_id"] = "split-narrative"
        result = self.run_validator(payload)
        self.assertEqual(result.returncode, 1)
        self.assertIn("must not preselect", result.stdout)
