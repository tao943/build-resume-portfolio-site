...
def valid_v2_report() -> tuple[dict, dict]:
    report = valid_report()
    report["schema_version"] = 2
    report["structure_selection"] = {
        "origin": "seed", "structure_seed_id": "split-narrative",
        "topology": {"axis": "vertical", "anchor": "sticky"},
        "invariants": ["desktop lanes remain unequal"],
        "variation_choices": ["thesis lane is narrow"],
        "responsive_transformations": ["linearize into chapters"],
        "anti_degeneracy_rules": ["do not collapse to uniform cards"],
        "motion_slots": ["evidence-progression"],
    }
    report["agent_selection"] = {
        "script_recommended_direction_id": "direction-fit",
        "final_direction_id": "direction-fit",
        "pairwise_rationale": ["fit beats novelty on content/form coherence", "fit beats wildcard on delivery risk"],
        "rejected_direction_reasons": {"direction-novelty": "weaker evidence hierarchy", "direction-wildcard": "higher responsive risk"},
        "blocking_floors_confirmed": True,
        "override_criterion": None,
    }
    report["novelty_brief"] = ["avoid the recent centered protagonist"]
    intelligence = {
        "candidate_directions": [
            {"id": "direction-fit", "blocking_floors": {"capacity": True, "responsive": True}},
            {"id": "direction-novelty", "blocking_floors": {"capacity": True, "responsive": True}},
            {"id": "direction-wildcard", "blocking_floors": {"capacity": True, "responsive": True}},
        ],
        "script_recommended_direction_id": "direction-fit",
    }
    report["selected_candidate_id"] = "direction-fit"
    report["layout_candidates"] = [
        {"id": "direction-fit", "family": "Split narrative", "fit": "strong hierarchy", "risks": ["lane tuning"], "responsive_fallback": "linear chapters"},
        {"id": "direction-novelty", "family": "Poster evidence", "fit": "strong novelty", "risks": ["copy pressure"], "responsive_fallback": "scaled poster"},
        {"id": "direction-wildcard", "family": "Wildcard field", "fit": "unusual topology", "risks": ["responsive risk"], "responsive_fallback": "ordered list"},
    ]
    return report, intelligence


class CreativeDirectionValidatorTests(unittest.TestCase):
    def run_validator(self, report: dict, design_intelligence: dict | None = None) -> subprocess.CompletedProcess[str]:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if design_intelligence is not None:
                intelligence_path = Path(temp_dir) / "design-intelligence.json"
                intelligence_path.write_text(json.dumps(design_intelligence), encoding="utf-8")
                command.extend(["--design-intelligence", str(intelligence_path)])
    ...
    def test_accepts_v2_agent_selection_linked_to_passing_direction(self) -> None:
        report, intelligence = valid_v2_report()
        result = self.run_validator(report, intelligence)
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)

    def test_v2_override_requires_semantic_criterion(self) -> None:
        report, intelligence = valid_v2_report()
        report["selected_candidate_id"] = "direction-novelty"
        report["agent_selection"]["final_direction_id"] = "direction-novelty"
        result = self.run_validator(report, intelligence)
        self.assertEqual(result.returncode, 1)
        self.assertIn("override_criterion", result.stdout)

    def test_v2_rejects_direction_with_failed_blocking_floor(self) -> None:
        report, intelligence = valid_v2_report()
        intelligence["candidate_directions"][0]["blocking_floors"]["responsive"] = False
        result = self.run_validator(report, intelligence)
        self.assertEqual(result.returncode, 1)
        self.assertIn("blocking floors", result.stdout)
