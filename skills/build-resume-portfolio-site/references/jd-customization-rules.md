...
Every matched row must link both fact IDs and evidence IDs and state the exact
resume location and rationale. Every unmatched row must keep both ID lists empty.
Validate the complete one-requirement-to-one-match report against
`jd-match-schema.json` with `scripts/validate_jd_match.py`.
